...
- Tests: `tests/smoke.sh` (launcher/install/herdr), `tests/model-selection.sh` (captain startup model chain), `tests/multi-project-captain.sh` (project resolution, real `bin/fm-spawn.sh` dispatch/isolation, instruction/skill separation, Pi/OMP routing - requires `tmux` and `treehouse` on `PATH`, uses an isolated `TMUX_TMPDIR` server so it never touches the shared/live session), `tests/doctor.sh` (`fm doctor` statuses/exit codes/JSON schema, fully fixture-isolated - see `tests/doctor.sh`'s own header for why its fixture PATH is hermetic rather than `$PATH`-appended). Run all four after touching `bin/fm`, `bin/fm-doctor`, `firstmate/*.md`, `firstmate/fm-captain-lib.sh`, or `install.sh`.
